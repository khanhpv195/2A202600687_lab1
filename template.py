...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        gpt4o_response = str(result.get("gpt4o_response", ""))
        mini_response = str(result.get("mini_response", ""))

        rows.append(
            [
                prompt if len(prompt) <= 40 else prompt[:37] + "...",
                (
                    gpt4o_response
                    if len(gpt4o_response) <= 40
                    else gpt4o_response[:37] + "..."
                ),
                (
                    mini_response
                    if len(mini_response) <= 40
                    else mini_response[:37] + "..."
                ),
